...
for particle in pop:
    particle.position = np.random.uniform(
        low=varMin, high=varMax, size=varSize)
...
for itr in range(maxIt):
    for i, particle in enumerate(pop):
        ...
        particle.velocity = (w * particle.velocity) + (c1 * r1 * (particle.best_position -
                                                                  particle.position)) + (c2 * r2 * (global_best_particle.position - particle.position))

        # apply velocity limits
        particle.velocity = np.array([max(p, velMin)
                                     for p in particle.velocity])
        particle.velocity = np.array([min(p, velMax)
                                     for p in particle.velocity])
